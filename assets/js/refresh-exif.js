...
async function startRefreshExif() {
    ...
    try {
        ...
        if (!startResponse.ok) {
            throw new Error('開始リクエストが失敗しました');
        }

        const startData = await startResponse.json();

        if (!startData.success) {
            throw new Error(startData.error || '開始に失敗しました');
        }
        ...
        if (!listResponse.ok) {
            throw new Error('ファイルリスト取得に失敗しました');
        }

        const listData = await listResponse.json();

        if (!listData.success) {
            throw new Error(listData.error || 'ファイルリスト取得に失敗しました');
        }
