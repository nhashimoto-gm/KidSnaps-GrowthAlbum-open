...
const translations = {
    en: {
        ...
        'file-info': 'Supported formats: JPEG, PNG, GIF, HEIC, MP4, MOV, AVI (Max 100MB per file, multiple files allowed)',
        'title-label': 'Title',
        ...
        'uploading': 'Upload',
        'file-size-error-alert': 'Files over 100MB are included. Please reselect the files.',
        'please-select-file': 'Please select a file.',
        'file-size-over-100mb': 'The following files exceed 100MB:',
    ...
    ja: {
        ...
        'file-select': 'ファイル選択',
        'file-info': '対応形式: JPEG, PNG, GIF, HEIC, MP4, MOV, AVI (各ファイル最大100MB、複数選択可)',
        'title-label': 'タイトル',
        ...
        'converting': '変換中...',
        'uploading': 'アップロード',
        'file-size-error-alert': '100MBを超えるファイルが含まれています。ファイルを選択し直してください。',
        'please-select-file': 'ファイルを選択してください。',
        'file-size-over-100mb': '以下のファイルは100MBを超えています:',
    ...
    uploadForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            if (imageFiles.length > 0) {
                if (uploadBtn) {
                    uploadBtn.innerHTML = `<span class="spinner-border spinner-border-sm me-2"></span>EXIF情報を読み取り中...`;
                }
...
function deleteMedia(mediaId, filename) {
    const lang = translations[currentLanguage];
    if (!confirm(lang['delete-confirm'].replace('{filename}', filename))) {
        return;
    }
