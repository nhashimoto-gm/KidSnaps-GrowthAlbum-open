...
function generateVideoThumbnail($videoPath, $thumbnailPath) {
    ...
    if (empty($ffmpegPath)) {
        error_log('ffmpegが見つかりません。./ffmpeg/ffmpeg' . ($isWindows ? '.exe' : '') . ' または システムPATHに配置してください。');
        return false;
    }
...
function importMediaFile($pdo, $filePath, $options) {
    try {
        ...
        if ($options['skipDuplicates'] && isFileRegistered($pdo, $filename, $filesize)) {
            error_log("重複スキップ: {$filename} (サイズ: {$filesize} bytes)");
            return ['status' => 'skipped', 'message' => "既に登録済み (ファイル名: {$filename}, サイズ: {$filesize})"];
        }
        ...
        if (getenv('DEBUG_MODE') === 'true') {
            error_log("=== データベース登録開始 ===");
            error_log("ファイル: {$filename}");
            error_log("パス: {$destPath}");
            error_log("タイプ: {$fileType}");
        }
        ...
        // デバッグログ: 登録成功
        if (getenv('DEBUG_MODE') === 'true') {
            error_log("登録成功 - ID: {$insertedId}");
            error_log("=== データベース登録完了 ===");
        }

        return ['status' => 'success', 'message' => "登録完了 (ID: {$insertedId})", 'id' => $insertedId];

    } catch (Exception $e) {
        $errorMsg = "インポートエラー ({$filePath}): " . $e->getMessage();
        error_log($errorMsg);
...
try {
    ...
    // ログファイルを作成
    $logFile = __DIR__ . '/../../logs/bulk_import_log_' . date('YmdHis') . '.txt';
    $logHandle = fopen($logFile, 'w');
    fwrite($logHandle, "=== Bulk Import Log ===\n");
    fwrite($logHandle, "開始時刻: " . date('Y-m-d H:i:s') . "\n");
    fwrite($logHandle, "対象ファイル数: {$totalFiles}\n");
    ...
    foreach ($mediaFiles as $index => $filePath) {
        ...
        $logEntry = "\n[{$current}/{$totalFiles}] " . basename($filePath) . "\n";
        $logEntry .= "  ファイルパス: {$filePath}\n";

        $result = importMediaFile($pdo, $filePath, $options);

        $logEntry .= "  結果: {$result['status']}\n";
        $logEntry .= "  メッセージ: {$result['message']}\n";

        if ($result['status'] === 'success') {
            $results['success']++;
            if (isset($result['id'])) {
                $successIds[] = $result['id'];
                $logEntry .= "  登録ID: {$result['id']}\n";
            }
    ...
    // ログファイルに結果サマリーを書き込む
    fwrite($logHandle, "\n" . str_repeat('=', 80) . "\n");
    fwrite($logHandle, "=== インポート結果 ===\n");
    fwrite($logHandle, "成功: {$results['success']}件\n");
    fwrite($logHandle, "スキップ: {$results['skipped']}件\n");
    fwrite($logHandle, "エラー: {$results['error']}件\n");
    if (!empty($successIds)) {
        fwrite($logHandle, "\n登録されたID一覧:\n");
        fwrite($logHandle, implode(', ', $successIds) . "\n");
    }
